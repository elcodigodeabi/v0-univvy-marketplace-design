...
export default function UsuariosRolesPage() {
  ...
  const [roleFilter, setRoleFilter] = useState<"todos" | "alumno" | "asesor">("todos")
  ...
  const updateUserRole = async (userId: string, newRole: "alumno" | "asesor") => {
    try {
        ...
        <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">Verifica y ajusta los roles de usuarios (alumnos/asesores)</p>
            </div>
          </div>
        </div>
      </div>
                    ...
                    {filteredUsers.map((user) => (
                      <tr key={user.id} className="border-b border-gray-100 hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3 text-center">
                          <Badge variant={user.role === "asesor" ? "default" : "secondary"}>
                            {user.role === "asesor" ? "Asesor" : "Alumno"}
                          </Badge>
                        </td>
                        <td className="px-4 py-3 text-center">
                          <Select
                            value={user.role}
                            onValueChange={(newRole) => updateUserRole(user.id, newRole as "alumno" | "asesor")}
